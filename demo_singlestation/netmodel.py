...
def sort_data(data,stn):
    ...
    tmp=sort_stn_xy(stn=stn)  ### 对台站进行排序按照x,y坐标大小
    stnx=tmp['stnx']
    stny=tmp['stny']
    #print(stnx,stny)
    data1=[]
    for i in range(len(stnx)):
        ...
        ## for循环放在前后有区别，因为放在后面会变成一个扁平的列表而不是嵌套的列表也就是 放后面数据是（12，1024，10）
        ##  ，放前面数据是（12*1024，10），减少了一个维度，所以放在后面
...
class NetModel():
    ...
    def predict_dl(self):
        ...
        windata=self.windata['windata'] #     ##    windata=self.datastream.get_data()
        xdata=[]
        ...
        dl_predict=self.model_dl.predict([loca_data,detec_data],batch_size=1, verbose=1) ### 神经网络预测
        ...
        return self.dl_re
    def dl_re_check(self,minpdf=0.0):  ###这段代码是用来检查预测的结果，如果预测的概率小于minpdf，则不进行预测，并且将预测的结果保存在dl_re_c中
        ...
        for dr in detec_predict:
            if dr[1]>minpdf:
                ...
                t0=[tb+dr[0]*self.sample_dt,dr[1],tb,idx] # t0=[t0,detec_prob,tbegin,idx]
    ...
    def predict_m_dlrec(self): ### 这个函数是用来预测地震的震级的，并且将预测的震级结果保存在mag_predict中
